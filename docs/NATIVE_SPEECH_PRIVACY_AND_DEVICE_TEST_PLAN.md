...
**Status:** Android private-pilot implementation in progress. The repository now contains a Capacitor Android app with an on-device, one-command speech adapter. iOS remains planning only.

**Decision for the private pilot:** do not add a cloud speech fallback, an always-on wake word, or a microphone service that starts from the background. The app now includes a user-started active-drive foreground service for local signal analysis; the separate one-command speech recognizer remains bounded and foreground-only. If a device cannot provide the approved on-device command path, Cantarivo keeps its button-first controls.
...
- [ ] Keep the current buttons: **Let companion join**, **Keep companion quiet**, and **End drive**. Voice is an additional input, not the only escape route.
- [ ] Make voice control opt-in for the current drive. The default is off. End Drive, microphone revocation, application termination, and device permission revocation all stop recognition and reset the next drive to **Ask before joining**.
- [ ] Show a parked-first explanation immediately before the operating-system prompt: "Cantarivo will listen on this device for four short commands during this drive: Companion join, Companion quiet, End drive, and Help. It does not save audio or transcripts, and it does not send them to a server."
...
- [ ] Show a persistent, plain-language state while voice control is active: "Voice controls active for this drive - on device - no recording." Include an immediate **Keep quiet** / **End drive** control.
...
- [ ] Keep a local, non-content receipt only: whether voice mode was enabled, when it stopped, and why. Clear it at drive end unless a later privacy policy and retention design explicitly permits otherwise.
- [ ] Do not make claims such as "hands-free safe," "always listening," or "works while you drive" until distraction testing supports the precise claim. Test only while parked or with a passenger/tester in a controlled, lawful setting.
...
**Preferred pilot: iOS 26+ only, SpeechAnalyzer.** Use `SpeechAnalyzer` with the on-device `SpeechTranscriber`/`SpeechDetector` path. Check model and language asset availability before allowing voice mode. Treat unavailable assets or unsupported language as a clean "Voice controls are unavailable on this device" state; keep buttons available.
...
<string>Cantarivo uses the microphone only during a drive you start to recognize four local voice commands. Audio is not recorded or saved.</string>
```

- [ ] Add this key **only if the shipping app calls `SFSpeechRecognizer.requestAuthorization`**. Apple requires it for that authorization call; omit the legacy recognizer entirely from a strict iOS 26 on-device-only pilot if it is not used.

```xml
<key>NSSpeechRecognitionUsageDescription</key>
<string>Cantarivo uses speech recognition only when you turn on voice controls for a drive, to recognize Companion join, Companion quiet, End drive, and Help.</string>
```

- [ ] Request microphone authorization only after the in-app explanation and the user selects **Turn on voice controls for this drive**. Check the result and offer buttons on denial, restriction, or later Settings revocation.
...
- [ ] Use only the approved locale and assets. Present asset/model download as a separate choice if a download is needed; do not start a drive in voice mode until the asset is ready.
- [ ] Stop audio input and cancel/finalize analysis on **Keep quiet**, **End drive**, app deactivation, audio interruption, route change, and microphone permission change. Release the analyzer and audio engine when stopped.
...
- [x] Provide the required ongoing foreground-service notification whenever the service runs, with Pause/Resume and End drive actions. Ask once for Android 13+ notification permission, but do not depend on that permission to start the service.
...
- [x] Use `SpeechRecognizer.createOnDeviceSpeechRecognizer(context)`, never the default `createSpeechRecognizer(context)`, in the local-only pilot.
...
- [ ] Do not build indefinite listening around Android's standard recognizer. Android states it is not intended for continuous recognition and can consume significant battery and bandwidth. The pilot must use bounded sessions, explicit lifecycle cleanup, and a separate technical review before any wake-word design.
- [x] Call `destroy()` on every recognizer instance when a command completes, the session times out, a permission error occurs, the app pauses, or the drive ends.
...
- [ ] Use a fresh install for each permission-path test. Do not test on a real drive. Park safely or use a bench environment.
- [ ] Test the same four phrases only: **Companion join**, **Companion quiet**, **End drive**, and **Help**. Document the expected action before testing.
- [ ] Test a small, repeatable sample: 20 deliberate repetitions of each command in each environment (quiet indoor, parked car with engine off, parked car with typical road-like noise, and Bluetooth connected). Record only aggregate pass/fail counts, not voice recordings or transcripts.
...
- [ ] Interrupt with a phone call, Siri, Bluetooth connect/disconnect, app backgrounding, screen lock, and a second start/stop cycle. Confirm audio input stops immediately and no recognition task survives End drive.
- [ ] Measure command action latency with a stopwatch from phrase end to visible state change. Establish a pilot threshold before recruiting anyone; reject a build with delayed, ambiguous, or unintended actions.

### Android acceptance tests

- [ ] Fresh install: verify the rationale screen, dynamic `RECORD_AUDIO` prompt, optional Android 13+ notification prompt, visible in-app listening state, ongoing active-drive notification, and immediate Pause/Resume and End drive controls.
...
- [ ] Put the app under Maps or another app after a valid visible start. Verify the drive notification and microphone indicator remain visible and local analysis continues. Verify notification Pause releases the microphone and Resume restores it without reopening parked setup.
...
- [ ] Verify End drive, Silence / Stop, permission revocation, and notification End drive release the microphone, remove the ongoing notification, stop companion output, and reset per-drive companion permission.
- [ ] Repeat the shared 20-per-command matrix and test Bluetooth connect/disconnect, screen lock, low battery, process pressure, and a second start/stop cycle. Verify the bounded recognizer `destroy()` leaves no stray recognizer while the separately authorized drive service follows its own notification state.
...
Do not invite external private-pilot testers until all of these are true:
...
3. **Later research, not a pilot shortcut:** a self-hosted/on-device wake-word or speech model. This can avoid a subscription, but it introduces model-license, distribution, battery, thermal, accessibility, and device-compatibility obligations. It needs a separate legal and technical evaluation before use.
